      ...
      renderItem={(plan, update) => (
        <div className="space-y-2">
          <div className="grid gap-2 sm:grid-cols-2">
            <MiniField label="الاسم" value={plan.name} onChange={(v) => update({ name: v })} />
            <MiniField label="السعر" ltr value={plan.price} onChange={(v) => update({ price: v })} />
            <MiniField
              label="الفترة"
              value={plan.period ?? ''}
        ...
        renderItem={(row, update) => (
          <div className="space-y-2">
            ...
            {block.columns.map((col) => {
              ...
              return (
                <MiniField
                  key={col}
                  label={col}
                  value={typeof raw === 'boolean' ? (raw ? 'نعم' : 'لا') : (raw ?? '')}
                  onChange={(v) =>
                    update({
                      // "نعم"/"لا" round-trip to the union's boolean form.
                      values: {
                        ...row.values,
                        [col]: v === 'نعم' ? true : v === 'لا' ? false : v,
                      },
                    })
                  }
                />
            ...
            {block.columns.length === 0 && (
              <p className="text-xs text-[var(--admin-text-muted)]">أضف الأعمدة أولاً.</p>
            )}
          </div>
        )}
      />
...
export function ProductGridEditor({
  ...
  return (
    <div className="space-y-3">
      <label className="block">
        <span className="mb-1 block text-xs text-[var(--admin-text-secondary)]">
          معرّفات المنتجات (سطر لكل معرّف)
        </span>
        <textarea
          rows={4}
          dir="ltr"
      ...
      <label className="block">
        <span className="mb-1 block text-xs text-[var(--admin-text-secondary)]">التخطيط</span>
        <select
      ...
      <p className="text-xs text-[var(--admin-text-muted)]">
        لا يظهر هذا القسم إلا إذا كانت وحدة المتجر مفعّلة في الإعدادات.
      </p>
    </div>
  );
}
...
export function CustomEditor({
  ...
  return (
    <div className="space-y-3">
      <label className="block">
        <span className="mb-1 block text-xs text-[var(--admin-text-secondary)]">اسم المكوّن</span>
        {available.length > 0 ? (
          <select
            className="admin-input"
      ...
      {available.length === 0 && (
        <p className="text-xs text-[var(--admin-warning)]">
          لا توجد مكوّنات مسجّلة. يسجّلها المطوّر في lib/blocks/custom-registry.tsx
        </p>
      )}
...
function PropsEditor({
  ...
  return (
    <label className="block">
      <span className="mb-1 block text-xs text-[var(--admin-text-secondary)]">الخصائص (JSON)</span>
      <textarea
        rows={5}
        dir="ltr"
        className="admin-input py-2 text-xs font-mono resize-y text-start"
