    ...
    <span className="inline-flex" role="img" aria-label={label}>
      {Array.from({ length: 5 }, (_, i) => (
        <Star
          ...
          size={15}
          aria-hidden="true"
          className={i < Math.round(value) ? 'fill-current text-amber-500' : 'text-gray-300'}
        />
      ))}
    </span>
  );
...
export function ProductReviews({
  ...
  return (
    <section className="mt-12 border-t border-gray-200 pt-8" data-test-id="product-reviews">
      ...
      {summary.count > 0 && (
        <p className="mt-3 flex items-center gap-2 text-sm text-gray-700" data-test-id="review-summary">
      ...
      {state === 'done' ? (
        <p className="mt-6 rounded-lg bg-green-50 p-4 text-sm text-green-800" role="status" data-test-id="review-thanks">
          {copy.thanks}
        </p>
      ) : (
        open && (
          <form onSubmit={submit} method="post" className="mt-6 grid gap-4 sm:grid-cols-2" data-test-id="review-form">
            ...
            <label className="text-sm">
              <span className="mb-1 block text-gray-700">{copy.name}</span>
              <input name="name" required minLength={2} className="w-full rounded-lg border border-gray-300 p-2.5" data-test-id="review-name" />
            </label>

            <label className="text-sm">
              <span className="mb-1 block text-gray-700">{copy.phone}</span>
              <input name="phone" required dir="ltr" className="w-full rounded-lg border border-gray-300 p-2.5 text-start" data-test-id="review-phone" />
            </label>

            <fieldset className="sm:col-span-2">
              <legend className="mb-1 text-sm text-gray-700">{copy.rating}</legend>
                ...
                {[1, 2, 3, 4, 5].map((n) => (
                  ...
                  >
                    <Star
                      size={22}
                      aria-hidden="true"
                      className={n <= rating ? 'fill-current text-amber-500' : 'text-gray-300'}
                    />
                  </button>
                ))}
              </div>
            ...
            <label className="text-sm sm:col-span-2">
              <span className="mb-1 block text-gray-700">{copy.body}</span>
              <textarea name="body" required minLength={10} rows={4} className="w-full rounded-lg border border-gray-300 p-2.5" data-test-id="review-body" />
            </label>

            <button
              type="submit"
              disabled={state === 'busy'}
              className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-5 py-2.5 text-sm font-medium text-white hover:bg-indigo-700 disabled:bg-gray-300"
      ...
      {reviews.length === 0 ? (
        <p className="mt-6 text-sm text-gray-500">{copy.none}</p>
      ) : (
        <ul className="mt-6 space-y-5">
          {reviews.map((review) => (
            <li key={review.id} className="border-b border-gray-100 pb-5 last:border-0" data-test-id={`review-item-${review.id}`}>
              ...
              <p className="mt-2 whitespace-pre-wrap text-sm text-gray-700">{review.body}</p>
            </li>
          ))}
        </ul>
      )}
