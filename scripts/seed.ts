...
async function seed() {
  ...
  /**
   * The demo catalogue, from the SAME module the setup wizard uses.
   *
   * It lived here as its own copy of the fixtures, which meant the wizard would
   * have needed a second copy — the seed script is not in the production image
   * at all. Two lists of demo products drift the first time either changes, so
   * there is now one, in lib/setup/demo.ts.
   */
  const demo = await installDemoContent();
  if (demo.products > 0) {
    console.log(`✅ Demo catalogue: ${demo.products} products across 2 brands and 2 categories`);
  }
